...
@Controller
@RequestMapping("/gatewayNode")
public class GatewayNodeController {
	
	private Logger log = Logger.getLogger(GatewayNodeController.class);
	...
	public Object queryFarmerMonitorGatewayList(HttpServletRequest request,Integer pageSize, Integer pageNumber){
		...
		try {
        	log.info("[GatewayNodeController:queryFarmerMonitorGatewayList]{pageSize:" + pageSize + ",pageNumber:" + pageNumber + "}query base info for page.");
			...
			log.error("[GatewayNodeController:queryFarmerMonitorGatewayList]false to query base list.", e);
		}
		return map;
	}
	...
	@ResponseBody
	public Object addSensorGateway(HttpServletRequest request,GatewayNode gatewayNode) {
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			log.info("[GatewayNodeController:addSensorGateway]add gatewayNode for page.");
			Long businessId = (Long) SessionUtil.getAttribute(request, SessionUtil.FARM_ID);
			gatewayNode.setBusinessId(businessId);
			gatewayNodeManager.insert(gatewayNode);
			map.put("success", true);
		} catch (Exception e) {
			map.put("success", false);
			log.error("[GatewayNodeController:addSensorGateway]false to add gatewayNode list.", e);
		}
		return map;
	}
